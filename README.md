...
정식 버전 태그가 없는 상태에서 main에 처음 push하면 커밋 제목과 관계없이 `v1.0.0` 태그와 Release를 생성합니다. 이후부터 위 접두사 규칙을 적용하며 `init:`는 별도 명령으로 취급하지 않습니다. 최초 여부는 브랜치 생성 이벤트나 커밋 수가 아니라 정식 버전 태그의 부재로 판별합니다. 기존 저장소에 워크플로를 도입한 경우도 같으며, 기존 태그가 있으면 초기화하지 않습니다.
