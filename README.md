...
`.github/workflows/release.yml`은 `main`에 push된 마지막 커밋의 제목을 확인하여 Git 버전 태그를 생성합니다. 이 문서는 현재 동작을 요약하고 Python, Node.js, Spring Boot, Kotlin 프로젝트의 버전 값을 같은 버전으로 갱신하는 확장 예제를 제공합니다.
...
| 태그가 없을 때 | `v1.0.0`을 기준값으로 사용 |
| 중복 방지 | 해당 커밋에 정식 버전 태그가 이미 있으면 정상 종료 |
| 권한 | 기본 `GITHUB_TOKEN`과 `contents: write` 사용 |
| 생성 결과 | push 대상 커밋을 가리키는 경량 Git 태그 |
...
태그가 전혀 없으면 첫 결과는 `major:` → `v2.0.0`, `minor:` → `v1.1.0`, `patch:` → `v1.0.1`입니다. `v1.0.0` 자체를 최초 태그로 자동 생성하는 방식은 아닙니다.
...
현재 워크플로는 프로젝트 버전 파일 수정, 빌드, 배포, GitHub Release 생성은 수행하지 않습니다.
...
현재 `release.yml`의 `case ... esac` 바로 다음, `git tag "$next_tag" HEAD` 바로 전에 아래 공통 코드와 해당 프로젝트의 갱신 코드를 넣습니다. 지원하지 않는 접두사는 앞선 `exit 0`에서 종료하므로 갱신 코드도 실행되지 않습니다.
...
실행 순서는 `버전 계산 → RELEASE_VERSION 설정 → 프로젝트 버전 갱신 → 필요한 검증/빌드 → 기존 태그 생성 및 push`입니다. 각 도구에서 `minor` 등을 다시 계산하지 말고, 태그 기준으로 계산한 정확한 버전을 전달합니다.
