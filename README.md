...
3. `detectProjType.sh`의 첫 줄 `${{ inputs.project_type }}`는 독립 Bash 파일에서 치환되지 않습니다. 워크플로에서 입력을 환경 변수로 전달하고 스크립트에서 읽도록 고칩니다.
4. Actions의 **Run workflow**에서 `project_type`(`auto`, `gradle`, `maven`, `node`, `python`)과 `bump`(`patch`, `minor`, `major`)를 선택합니다.

`auto`는 루트의 `build.gradle`, `pom.xml`, `package.json`, `pyproject.toml`을 찾습니다. 파일이 없거나 둘 이상이면 실패합니다. 여러 종류가 있는 프로젝트는 타입을 명시해야 합니다.
